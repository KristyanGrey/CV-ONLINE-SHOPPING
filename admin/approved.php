...
if(isset($_POST['submit'])) {
 ...
 if(gettype($_POST['id'])=="array"){
    foreach($_POST['id'] as $val){
        $id_c=$val;
        $sql = "UPDATE orders SET status='shipped' WHERE id='".$id_c."'";
...
<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <ul class="treeview-menu">
            ...
            <li class="active"><a href="#"><i class="fa fa-circle-o"></i> Approved</a></li>
            ...
            <div class="box box-info">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    $sql = "SELECT * FROM ".$datatable." WHERE status='processing' ORDER BY created DESC LIMIT $start_from, ".$results_per_page;
                                    $rs_result = $conn->query($sql);
                                    if($row = $rs_result->fetch_assoc()) {
                                        ?>
                                        <tr>
                                            <td><center><?php echo $row["customer_id"];  
                                                $_SESSION['customer_id']= $row["customer_id"];?></center></td>
                                                <td><center>&#8369;<?php echo $row["total_price"];
                                                    $_SESSION['total']=$row["total_price"];?></center></td>
                                                    <td><center><?php echo $row["created"]; ?></center></td>
                                                    <td><center><?php if($row["status"] == 'processing'){ echo " <span class='label label-info'>Processing</span>";} ?></center></td>
                                ...
                                <?php 
                                $sql = "SELECT COUNT(id) AS total FROM ".$datatable." WHERE status='processing' ORDER BY created DESC";
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-block btn-info" name="submit">Submit</button>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <center>
                        ...
                        for ($i=1; $i<=$total_pages; $i++) {  // print links for all pages
                            echo "<button class='btn'><a href='approved.php?page=".$i."'";
